...
export class DemoMeetingApp implements AudioVideoObserver, PingPongObserver {
  ...
  constructor() {
    ...
    setTimeout(() => {
      ...
      this.sendStatus(JSON.stringify({metricName: 'MeetingJoin', metricValue: 1, iId: this.instanceId, ltStartTime: this.loadTestStartTime}));
  ...
  initEventListeners(): void {
    ...
    buttonMeetingLeave.addEventListener('click', _e => {
      new AsyncScheduler().start(async () => {
        ...
        //this.log(JSON.stringify({MeetingLeave: 1, instanceId: this.instanceId}));
        this.sendStatus(JSON.stringify({metricName: 'MeetingLeave', metricValue: 1, iId: this.instanceId, ltStartTime: this.loadTestStartTime}));
    ...
    setInterval(() => {
      if (new Date().getMinutes() % 5 === 0 && this.meetingActive === true) {
        ...
        this.sendStatus(JSON.stringify({metricName: 'alivePing', metricValue: 1, iId: this.instanceId, ltStartTime: this.loadTestStartTime}));
  ...
  async sendStatus(body: string): Promise<void> {
    try {
      const response = await fetch(`${DemoMeetingApp.BASE_URL}send_status`, {
        method: 'POST',
        body
      });
      if (response.status === 200) {
  ...
  async initializeMeetingSession(configuration: MeetingSessionConfiguration): Promise<void> {
    ...
    this.log('InstanceId:' + this.instanceId + ' meetingId: ' + this.sessionMeetingId + ' attendeeId: ' + this.sessionAttendeeId);
  }
  ...
  async metricsDidReceive(clientMetricReport: ClientMetricReport): Promise<void> {
    ...
    if (typeof metricReport.audioPacketsReceived === 'number') {
      ...
      this.metricReport = {
        ...
        iId: this.instanceId,
        ltStartTime: new Date(parseInt(this.loadTestStartTime.toString())),
        mId: this.sessionMeetingId,
        aId: this.sessionAttendeeId
      }
      //console.log(this.metricReport);
    } else {
      this.log(`{'...empty...'}`);
  ...
  async endMeeting(): Promise<any> {
    ...
    this.sendStatus(JSON.stringify({metricName: 'MeetingDelete', metricValue: 1, iId: this.instanceId, ltStartTime: this.loadTestStartTime}));
  ...
  audioVideoDidStartConnecting(reconnecting: boolean): void {
    if (reconnecting === true) {
      ...
      this.sendStatus(JSON.stringify({metricName: 'ReconnectingSession', metricValue: 1, iId: this.instanceId, ltStartTime: this.loadTestStartTime}));
      //this.log(JSON.stringify({ReconnectingMeeting: 1, instanceId: this.instanceId}));
    } else {
      this.log('ConnectingSession');
      this.sendStatus(JSON.stringify({metricName: 'ConnectingSession', metricValue: 1, iId: this.instanceId, ltStartTime: this.loadTestStartTime}));
  ...
  audioVideoDidStart(): void {
    ...
    this.sendStatus(JSON.stringify({metricName: 'MeetingStarted', metricValue: 1, iId: this.instanceId, ltStartTime: this.loadTestStartTime}));
  ...
  audioVideoDidStop(sessionStatus: MeetingSessionStatus): void {
    ...
    this.sendStatus(JSON.stringify({metricName: 'StatusCode-' + statusCode, metricValue: 1, iId: this.instanceId, ltStartTime: this.loadTestStartTime}));
